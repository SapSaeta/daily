    ...
    <div className="px-4 pt-6 pb-4 space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-slate-100 capitalize">{today}</h1>
        </div>
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-sky-500/20">
      ...
      <div>
        ...
        {data.todayPlanned.length > 0 ? (
          ...
          <div className="card text-center py-6">
            ...
            <p className="text-sm text-slate-400 mb-1">Sin plan para hoy</p>
            <p className="text-xs text-slate-600 mb-4">Genera un plan semanal para ver tus entrenos</p>
            <Link href="/plan" className="btn-primary inline-block text-sm py-2 px-5">
              Generar plan
            </Link>
          </div>
        )}
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <Link
            ...
            className="card flex items-center gap-3 active:bg-slate-800 transition-colors"
          >
            ...
            <div>
              <p className="text-sm font-semibold text-slate-100">Natación</p>
              <p className="text-xs text-slate-500">Registrar sesión</p>
            </div>
          </Link>
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-slate-100">Bandas</p>
              <p className="text-xs text-slate-500">Registrar sesión</p>
            </div>
          </Link>
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-slate-100">Peso</p>
              <p className="text-xs text-slate-500">
                {data.currentWeight != null ? `${data.currentWeight} kg` : 'Registrar'}
              </p>
            </div>
          </Link>
        </div>
      ...
      {data.recentSessions.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/historial" className="text-xs text-sky-400 hover:text-sky-300">
              Ver todo
            </Link>
          </div>
          <div className="space-y-2">
            {data.recentSessions.map((session, i) => (
              <div key={i} className="card-sm flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  session.type === 'swim' ? 'bg-sky-500/10' : 'bg-violet-500/10'
                }`}>
                  {session.type === 'swim' ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 truncate">{session.detail}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-[10px] text-slate-500 capitalize">{formatDate(session.date)}</p>
                  {session.pain > 0 && (
                    <span className={`text-xs font-medium ${
                      session.pain <= 2 ? 'text-emerald-400' :
                      session.pain <= 4 ? 'text-amber-400' :
                      session.pain <= 6 ? 'text-orange-400' :
                      'text-red-400'
                    }`}>
                      Dolor {session.pain}/10
                    </span>
                  )}
                </div>
              </div>
      ...
      <Link href="/ejercicios" className="card flex items-center gap-3 active:bg-slate-800 transition-colors">
        <div className="w-9 h-9 rounded-xl bg-emerald-500/10 flex items-center justify-center flex-shrink-0">
          <svg className="w-5 h-5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-slate-100">Biblioteca de ejercicios</p>
          <p className="text-xs text-slate-500">Ejercicios para el hombro y calentamientos</p>
        </div>
